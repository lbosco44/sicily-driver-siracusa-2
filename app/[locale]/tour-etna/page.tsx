...
export async function generateMetadata({
  ...
  const tour = getTour('etna', locale as Locale);
  const itPath = '/it/tour-etna';
  const enPath = '/en/etna-tour';
  return {
...
export default async function TourEtnaPage({
  params
}: {
  params: Promise<{locale: string}>;
}) {
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbSchema(getBreadcrumb('tour-etna', locale as Locale, tour.h1))}
      />
